...
export const createBeachCouple = (): BeachCoupleRig => {
  ...
  block([3.8, 0.08, 5.4], [0, 0.04, 0.35], towelCream, root);
  for (const z of [-1.7, -0.5, 0.7, 1.9, 2.6]) {
    block([3.82, 0.09, 0.46], [0, 0.05, z], towelRed, root);
  }
  ...
  const person = (
    ...
    block([1.0, 0.52, 1.5], [0, 0.42, -0.1], costume, group);
    block([0.98, 0.5, 0.5], [0, 0.42, 0.75], skin, group);

    // Head on a pivot at the shoulder end, so it can turn in without the body
    // following it.
    const head = new THREE.Group();
    head.position.set(0, 0.5, -1.02);
    group.add(head);
  ...
  block([3.5, 0.22, 3.5], [0, 3.4, 0], canopy, parasol);
  block([1.7, 0.26, 3.55], [0, 3.39, 0], canopyCream, parasol);
  block([3.55, 0.26, 1.7], [0, 3.38, 0], canopyCream, parasol);
  block([0.8, 0.3, 0.8], [0, 3.56, 0], canopy, parasol);
